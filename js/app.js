'use strict';
//Getting User name
//Funtion to get username and check for  null and empty string and run loop untill name entered.
...
// random value generated
  ...
  var userGuessedNumber = prompt('We have selected a random number between 1 - 15. See if you can guess it in 4 Trys.');
  while(guess < 4){
    if(randomNumber === parseInt(userGuessedNumber))
    {
      alert('CONGRATULATIONS!!! YOU GUESSED IT RIGHT IN '+ guess + ' GUESS ');
      break;
    } else if(randomNumber < parseInt(userGuessedNumber)) {/* if guessed number is greater than actual number*/
      console.log('this is the random number',randomNumber);
      userGuessedNumber = prompt('OOPS SORRY!! TRY A SMALLER NUMBER');
      guess++;
    } else if(randomNumber > parseInt(userGuessedNumber)){ /* if guessed number is Smaller than actual number*/
